...
workflow PREPARE_GENOME {
    ...
    //
    if (params.fasta_genes.endsWith('.gz')) {
        ch_fasta    = GUNZIP_FASTA ( [ [:], params.fasta_genes ] ).gunzip.map { it[1] }
        ch_versions = ch_versions.mix(GUNZIP_FASTA.out.versions)
    } else {
        ch_fasta = file(params.fasta_genes)
    }
    ...
    //
    if (params.gtf_genes) {
        if (params.gtf_genes.endsWith('.gz')) {
            ch_gtf      = GUNZIP_GTF ( [ [:], params.gtf_genes ] ).gunzip.map { it[1] }
            ch_versions = ch_versions.mix(GUNZIP_GTF.out.versions)
        } else {
            ch_gtf = file(params.gtf_genes)
        }
    } else if (params.gff_genes) {
        if (params.gff_genes.endsWith('.gz')) {
            ch_gff      = GUNZIP_GFF ( [ [:], params.gff_genes ] ).gunzip.map { it[1] }
            ch_versions = ch_versions.mix(GUNZIP_GFF.out.versions)
        } else {
            ch_gff = file(params.ch_gff_genes)
        }
    ...
    //
  /*  if (params.genesgtf) {
        if (params.genesgtf.endsWith('.gz')) {
...
*/
    ...
    //
    if (params.transcript_fasta_genes) {
        if (params.transcript_fasta_genes.endsWith('.gz')) {
            ch_transcript_fasta = GUNZIP_TRANSCRIPT_FASTA ( [ [:], params.transcript_fasta_genes ] ).gunzip.map { it[1] }
            ch_versions         = ch_versions.mix(GUNZIP_TRANSCRIPT_FASTA.out.versions)
        } else {
            ch_transcript_fasta = Channel.value(file(transcript_fasta))
        }
        if (params.gencode_genes) {
    ...
    if ('hisat2' in prepare_tool_indices) {
        ...
        if (params.hisat2_index_genes) {
            if (params.hisat2_index_genes.endsWith('.tar.gz')) {
                ch_hisat2_index = UNTAR_HISAT2_INDEX ( [ [:], params.hisat2_index_genes ] ).untar.map { it[1] }
                ch_versions     = ch_versions.mix(UNTAR_HISAT2_INDEX.out.versions)
            } else {
                ch_hisat2_index = file(params.hisat2_index_genes)
            }
        } else {
    ...
    if ('star' in prepare_tool_indices) {
        if (params.star_index_genes) {
            if (params.star_index_genes.endsWith('.tar.gz')) {
                UNTAR_STAR_INDEX (
                    Channel.fromPath(params.star_index_genes).map{ it -> [[id:it[0].baseName], it] }
                )
                ch_star_index = UNTAR_STAR_INDEX.out.untar.map{ meta, star_index -> [star_index] }.collect()
                ch_versions   = ch_versions.mix(UNTAR_STAR_INDEX.out.versions)
            } else {
                ch_star_index = Channel.fromPath(params.star_index_genes).collect()
            }
        }
        else {
            STAR_GENOMEGENERATE (
...
/*         if((!ch_star_index) || getIndexVersion(ch_star_index) != '2.7.4a'){
    ...
    if (params.salmon_index_genes) {
        if (params.salmon_index_genes.endsWith('.tar.gz')) {
            ch_salmon_index = UNTAR_SALMON_INDEX ( [ [:], salmon_index_genes ] ).untar.map { it[1] }
