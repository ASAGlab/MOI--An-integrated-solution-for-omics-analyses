...
WorkflowAlignassemblymirna.initialise(params, log)


// Check input path parameters to see if they exist
checkPathParamList = [
    params.salmon_index_mirna
]
...
if (!params.skip_bbsplit_mirna)   { prepareToolIndices << 'bbsplit'             }
if (!params.skip_alignment_mirna) { prepareToolIndices << params.aligner_mirna        }
if (!params.skip_pseudo_alignment_mirna && params.pseudo_aligner_mirna) { prepareToolIndices << params.pseudo_aligner_mirna }
...
if (params.fasta_mirna && params.gtf_mirna) {
    if ((file(params.fasta_mirna).getName() - '.gz' == 'genome.fa') && (file(params.gtf_mirna).getName() - '.gz' == 'genes.gtf')) {
        is_aws_igenome = true
    }
}

if(params.mirna){
    if (params.input_mirna) { ch_input = file(params.input_mirna) } else { exit 1, 'Input samplesheet not specified!' }
}
...
include { PREPARE_GENOME } from '../subworkflows/local/prepare_genome_mirna'
...
include { QUANTIFY_SALMON as QUANTIFY_SALMON_MIRNA      } from '../subworkflows/local/quantify_salmon'
...
include { SALMON_TXIMPORT as SALMON_TXIMPORT_MIRNA } from '../modules/local/salmon_tximport'
...
params.fasta_mirna           = WorkflowAlignassemblymirna.getGenomeAttribute(params, 'fasta')
params.transcript_fasta_mirna= WorkflowAlignassemblymirna.getGenomeAttribute(params, 'transcript_fasta')

params.gtf_mirna             = WorkflowAlignassemblymirna.getGenomeAttribute(params, 'gtf')

params.hisat2_index_mirna     = WorkflowAlignassemblymirna.getGenomeAttribute(params, 'hisat2')

params.salmon_index_mirna    = WorkflowAlignassemblymirna.getGenomeAttribute(params, 'salmon')
params.fasta_salmon_mirna    = WorkflowAlignassemblymirna.getGenomeAttribute(params, 'transcript_fasta')
...
workflow ALIGNASSEMBLYMIRNA {
    ...
    //
    def biotype = params.gencode_mirna ? "gene_type" : params.featurecounts_group_type_mirna
    PREPARE_GENOME (
        prepareToolIndices, biotype, is_aws_igenome,params.salmon_index_mirna
    )
    ...
    ch_gtf_mirna = PREPARE_GENOME.out.gtf
    if (!params.skip_pseudo_alignment_mirna && params.pseudo_aligner_mirna == 'salmon') {
        QUANTIFY_SALMON_MIRNA (
            fr,
        ...
        ch_salmon_multiqc = QUANTIFY_SALMON_MIRNA.out.results
        ...
        counts_from_abundance_tximport         = SALMON_TXIMPORT_MIRNA.out.countsfromabundance
        salmon_dgelistRdata_tximport           = SALMON_TXIMPORT_MIRNA.out.salmon_dgelistRdata
    }
    //
    // SUBWORKFLOW: Alignment with HISAT2
    //
    ch_hisat2_multiqc = Channel.empty()
    if (!params.skip_alignment_mirna && params.aligner_mirna == 'hisat2') {
        ALIGN_HISAT2 (
            fr,
...
/////  SUBWORKFLOW STAR
    ...
    if (!params.skip_alignment_mirna && params.aligner_mirna == 'star') {
    ...
    if (!params.skip_alignment_mirna && !params.skip_qc_mirna && !params.skip_preseq_mirna) {
        PRESEQ_LCEXTRAP (
            ch_genome_bam
        )
    ...
    if (!params.skip_alignment_mirna && !params.skip_markduplicates_mirna) {
    ...
    if (!params.skip_alignment_mirna && !params.skip_stringtie_mirna) {
        STRINGTIE_STRINGTIE (
            ch_genome_bam,
            PREPARE_GENOME.out.gtf
        )
...
//    CUT(ch_abundance_stringtie,2,9) // put it as different name to run indipendetly
    ...
    if (!params.skip_alignment_mirna && params.skip_pseudo_alignment_mirna && !params.aligner_mirna=='star'){
...
else if(!params.skip_alignment_mirna && params.aligner_mirna=='star'){
    CUT(ch_genestar1,ch_genestar,1,2)
    CONCETRATE(CUT.out.counts.map { meta, file -> return[file]}.collect().view())
    ch_concentrated_matrix = CONCETRATE.out.count_matrix
}

    if(!params.skip_pseudo_alignment_mirna){
        ch_genome_bam=Channel.empty()
    }
