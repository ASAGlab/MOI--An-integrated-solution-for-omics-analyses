...
title="PCA of treatment and replicate"
...
#args4<-args[4]
#args5<-args[5]
#samplesinfo$treatment<-samplesinfo$args4
#samplesinfo$replicate<-samplesinfo$args5
#print(head(samplesinfo))
vec1<-which(colnames(samplesinfo)==as.character(args[4]))
vec2<-which(colnames(samplesinfo)==as.character(args[5]))
colnames(samplesinfo)[vec1] <- "treatment"
colnames(samplesinfo)[vec2] <- "replicate"


samplesinfo[] <- lapply(samplesinfo, factor)
print(head(samplesinfo))
#print(samplesinfo$replicate)
print("pca of raw")
svg("pca_raw.svg")
PreparePlotpca2(samples,samplesinfo,"treatment","replicate")
dev.off()
...
if (args[3]=="svacom"){
  ...
  cleaned_svacom=svacom(samples,samplesinfo,"treatment","replicate")
  write.table(cleaned_svacom,"cleaned_svacom.txt",sep="\t", quote=F)
  #svg("svacom.svg")
  title=paste0("PCA of ", as.character(args[4]), " and ", as.character(args[5]) ,".")
  svg("svacom.svg")
  PreparePlotpca2(cleaned_svacom,samplesinfo,"treatment","replicate")
  dev.off()
}
if (args[3]=="comsva"){
  print("clean with combat and sva")
  cleaned_comsva=comsva(samples,samplesinfo,"treatment","replicate")
  write.table(cleaned_comsva,"cleaned_comsva.txt",sep="\t", quote=F)
  #svg("comsva.svg")
  title=paste0("PCA of ", as.character(args[4]), " and " ,as.character(args[5]), ".")
  svg("comsva.svg")
  #PreparePlotpca2(cleaned_comsva,samplesinfo,args[4],as.factor(args[5]))
  PreparePlotpca2(cleaned_comsva,samplesinfo,"treatment","replicate")
  dev.off()
}
if (args[3]=="com"){
  print("clean with combat")
  cleaned_com=com(samples,samplesinfo,"replicate")
  write.table(cleaned_com,"cleaned_com.txt",sep="\t", quote=F)
  #svg("com.svg")
  title=paste0("PCA of ", as.character(args[4]), " and " ,as.character(args[5]), ".")
  svg("com.svg")
  PreparePlotpca2(cleaned_comsva,samplesinfo,"treatment","replicate")
...
if (args[3]=="sva"){
  ...
  cleaned_sva=svAll(samples,samplesinfo,args[4],args[5])
  write.table(cleaned_sva,"cleaned_sva.txt",sep="\t", quote=F)
  title=paste0("PCA of " ,as.character(args[4]), " and " ,as.character(args[5]), ".")
  svg("sva.svg")
  PreparePlotpca2(cleaned_comsva,samplesinfo,"treatment","replicate")
...
if (args[3]=="all"){
  print("clean with sva and combat")
  cleaned_svacom=svacom(samples,samplesinfo,args[4],args[5])
  write.table(cleaned_svacom,"cleaned_svacom.txt",sep="\t", quote=F)
  svg("svacom.svg")
  PreparePlotpca2(cleaned_svacom,samplesinfo,args[4],as.factor(args[5]))
  dev.off()
  print("clean with combat and sva")
  cleaned_comsva=comsva(samples,samplesinfo,args[4],args[5])
  write.table(cleaned_comsva,"cleaned_comsva.txt",sep="\t", quote=F)
  svg("comsva.svg")
  PreparePlotpca2(cleaned_comsva,samplesinfo,args[4],as.factor(args[5]))
  dev.off()
  print("clean with combat")
  cleaned_com=com(samples,samplesinfo,args[5])
  write.table(cleaned_com,"cleaned_com.txt",sep="\t", quote=F)
  svg("com.svg")
  PreparePlotpca2(cleaned_com,samplesinfo,args[4],as.factor(args[5]))
  dev.off()
  print("clean with sva")
  cleaned_sva=svAll(samples,samplesinfo,args[4],args[5])
  write.table(cleaned_sva,"cleaned_sva.txt",sep="\t", quote=F)
